...
fn init_ballots() -> Vec<Ballot> {
    ...
    for _ in 0..50 {
        ballots.push(Ballot::new(Scalar::ONE, Scalar::ZERO, Scalar::ZERO, &mut rng)); // Alice
    }
    for _ in 0..50 {
        ballots.push(Ballot::new(Scalar::ZERO, Scalar::ONE, Scalar::ZERO, &mut rng)); // Bob
    }
    for _ in 0..60 { // Increase Carol's ballots to 60
        ballots.push(Ballot::new(Scalar::ZERO, Scalar::ZERO, Scalar::ONE, &mut rng)); // Carol
    }
    ballots
}
